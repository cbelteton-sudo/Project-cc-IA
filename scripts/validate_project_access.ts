...
async function runTests() {
  ...
  try {
    ...
    if (!project) throw new Error('Project TORRE MAWI DEMO not found. Seed first.');
    ...
    // Login Admin
    await client.post('/auth/login', { email: ADMIN_EMAIL, password: PASSWORD });
    ...
    // Get Members
    const adminMembersRes = await client.get(`/admin/projects/${project.id}/members`);
    if (adminMembersRes.status === 200) {
      console.log(`   ✅ Admin can list members (${adminMembersRes.data.length} members)`);
    }

    // 2. Intruder Access Test (No Membership)
    console.log('\n2️⃣  Testing Intruder Access (No Membership)...');
    // Login Intruder
    await client.post('/auth/login', { email: INTRUDER_EMAIL, password: PASSWORD });
    ...
    const hasProject = intruderListRes.data.some((p: any) => p.id === project.id);
    if (!hasProject) {
    ...
    try {
      ...
      console.error('   ❌ Intruder ACCESSED admin members endpoint (Should be 403)');
      process.exit(1);
    } catch (e: any) {
      if (e.response && e.response.status === 403) {
        console.log('   ✅ Intruder denied access to admin endpoint (403)');
      } else {
        console.warn('   ⚠️ Unexpected error on admin endpoint:', e.message, e.response?.status);
        // Note: RolesGuard might return 403 as well.
      }
    }
    ...
    const hasProjectNow = grantedListRes.data.some((p: any) => p.id === project.id);
    if (hasProjectNow) {
      console.log('   ✅ Intruder now sees T-MAWI in project list');
    } else {
      console.error('   ❌ Intruder DOES NOT SEE T-MAWI in project list (List filtering failed?)');
    }

    // 5. Revoke Access
    console.log('\n5️⃣  Testing Access Revocation...');
    // Login Admin
    await client.post('/auth/login', { email: ADMIN_EMAIL, password: PASSWORD }); // Re-login strictly
    ...
    try {
      await client.get(`/projects/${project.id}`);
      console.error('   ❌ Intruder ACCESSED project after removal (Should be 403)');
      process.exit(1);
    } catch (e: any) {
      if (e.response && e.response.status === 403) {
