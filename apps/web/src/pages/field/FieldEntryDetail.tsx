...
export const FieldEntryDetail: React.FC = () => {
    ...
    // We assume the user is reporting 'IN_PROGRESS' by default unless they explicitly want to close it (maybe a checkbox later)
    // For now, let's keep it simple: Just reporting progress. The status is derived or updated separately if needed.
    // User asked for "Read Only Status" buttons.
    ...
    useEffect(() => {
        const loadData = async () => {
            ...
            try {
                ...
                if (isOnline) {
                    try {
                        ...
                        if (act) {
                            if (act.parentId) {
                                try {
                                    const pRes = await axios.get(`${API_URL}/activities/${act.parentId}`, { headers: { Authorization: `Bearer ${token}` } });
                                    act.parentName = pRes.data.name;
                                } catch (e) { }
                            }
                            if (state?.projectId) {
                                try {
                                    const projRes = await axios.get(`${API_URL}/projects/${state.projectId}`, { headers: { Authorization: `Bearer ${token}` } });
                ...
                const localItems = await Promise.all(localEntries.map(async (e: any) => {
    ...
    const handleSave = async () => {
        ...
        try {
            ...
            // 1. Daily Report
            let report = await db.getFromIndex('field_daily_reports', 'by-project-date', [projectId, dateStr] as any);
            if (!report) {
                report = {
            ...
            // Sync
            await SyncQueue.add('/field/entries', 'POST', entryData); // Simplified sync
    ...
    const getScheduleStatus = () => {
        ...
        if (activity.status === 'DONE') return { text: 'COMPLETADO', color: 'bg-green-100 text-green-700 ring-green-600/20' };
        if (today > end) return { text: 'ATRASADO', color: 'bg-red-100 text-red-700 ring-red-600/20' };
        if (today >= start && today <= end) return { text: 'EN TIEMPO', color: 'bg-blue-100 text-blue-700 ring-blue-600/20' };
    ...
    if (!activity) return <div className="p-10 text-center text-gray-500">Actividad no encontrada</div>;
    ...
    return (
        <div className="flex flex-col h-full bg-gray-50">
                ...
                <div className="flex items-center gap-3">
                    ...
                    <div className="flex-1 min-w-0">
                        ...
                        <div className="flex items-center gap-2">
                            ...
                            <div className={`px-2 py-0.5 rounded-full text-[10px] font-bold ring-1 ring-inset shrink-0 ${schedule.color}`}>
                                {schedule.text}
                            </div>
                        </div>
                    </div>
                    ...
                    <div className="text-right shrink-0 pl-2 border-l border-gray-100 hidden sm:block">
                        <div className="text-[10px] uppercase tracking-wider font-bold text-gray-400">Fecha Fin</div>
                        <div className="text-sm font-semibold text-gray-700">{format(new Date(activity.endDate), 'dd MMM')}</div>
                    </div>
                </div>
            ...
            <div className="bg-white px-4 border-b border-gray-100 flex gap-8 z-10 flex-none">
                ...
                >
                    Reporte
                    {activeTab === 'REPORT' && <div className="absolute bottom-0 left-0 right-0 h-0.5 bg-black rounded-t-full" />}
                </button>
                <button
                ...
                >
                    Historial
                    {activeTab === 'HISTORY' && <div className="absolute bottom-0 left-0 right-0 h-0.5 bg-black rounded-t-full" />}
                </button>
            </div>
            ...
            <div className="flex-1 overflow-y-auto p-4 sm:p-6 bg-gray-50">
                {activeTab === 'REPORT' ? (
                    <div className="space-y-8 max-w-3xl mx-auto pb-6">
                        ...
                        <div className="bg-white p-6 rounded-2xl shadow-sm border border-gray-100">
                            <label className="text-base font-bold text-gray-800 mb-4 flex items-center gap-2">
                                <span className="bg-blue-100 p-2 rounded-lg text-blue-600"><FileText size={20} /></span>
                                Notas del Día
                            </label>
                            <textarea
                        ...
                        <div className="bg-white p-6 rounded-2xl shadow-sm border border-gray-100">
                            <label className="text-base font-bold text-gray-800 mb-4 flex items-center gap-2">
                                <span className="bg-purple-100 p-2 rounded-lg text-purple-600"><Camera size={20} /></span>
                                Evidencia
                            </label>

                            <div className="grid grid-cols-2 sm:grid-cols-3 gap-4">
                                {tempPhotos.map(p => (
                                    <div key={p.id} className="aspect-[4/3] relative rounded-xl overflow-hidden group shadow-sm ring-1 ring-gray-100">
                                        <img src={URL.createObjectURL(p.blob)} className="w-full h-full object-cover transition-transform group-hover:scale-105" />
                                        <button
                                ...
                                <label className="aspect-[4/3] flex flex-col items-center justify-center border-2 border-dashed border-gray-200 rounded-xl cursor-pointer hover:bg-gray-50 hover:border-blue-300 transition-all text-gray-400 bg-gray-50/50 group">
                                    <Camera size={32} className="mb-2 opacity-50 group-hover:text-blue-500 group-hover:opacity-100 transition-all" />
                                    <span className="text-xs font-bold uppercase tracking-wider group-hover:text-blue-600">Agregar Foto</span>
                    ...
                    <div className="space-y-4 max-w-3xl mx-auto">
                        ...
                        {logItems.length === 0 && <div className="text-center text-gray-400 py-10">Sin reportes aún</div>}
                    </div>
                )}
            </div>
