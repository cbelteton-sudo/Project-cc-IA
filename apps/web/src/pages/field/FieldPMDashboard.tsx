      ...
      {loading ? (
        ...
        <div className="p-8 text-center text-red-500 bg-white rounded-xl border border-red-100 shadow-sm">
          ...
          <button
            ...
            className="px-5 py-2 bg-red-50 text-red-600 rounded-lg text-sm font-semibold hover:bg-red-100 transition-colors"
          >
            Reintentar
          </button>
        </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 h-full">
            ...
            <div className="lg:col-span-1 space-y-6">
              <DashboardCard
                ...
                className="border-red-100 ring-4 ring-red-50"
              >
                <div className="space-y-3">
                  {metrics.blocked.items.length === 0 ? (
                    ...
                    metrics.blocked.items.map((item, idx) => (
                      <div
                        key={idx}
                        className="group p-3 bg-red-50/50 rounded-lg border border-red-100 hover:bg-red-50 transition-colors cursor-default"
                      >
                  ...
                  {metrics.stalled.items.length === 0 ? (
                    ...
                    metrics.stalled.items.map((item, idx) => (
                      ...
                      >
                        ...
                        <div
                          className={`absolute left-0 top-0 bottom-0 w-1 ${item.isBlocked ? 'bg-red-500' : item.highIssues > 0 ? 'bg-red-400' : 'bg-transparent transition-colors group-hover:bg-blue-400'}`}
                        ></div>

                        <div className="flex justify-between items-start gap-2 pl-2">
                          <div>
                            <div
                              className="font-medium text-gray-800 text-sm group-hover:text-blue-700 transition-colors line-clamp-2"
                              title={item.name}
                            >
                              {item.name}
                            </div>
...
const DashboardCard = ({ title, icon, children, className = '' }: any) => (
  <div
    className={`bg-white rounded-xl shadow-[0_2px_8px_rgba(0,0,0,0.04)] border border-gray-100 p-5 ${className}`}
  >
...
const StatCard = ({ label, value, trend, trendColor, icon, bg }: any) => (
  <div
    className={`${bg} p-5 rounded-xl shadow-[0_2px_8px_rgba(0,0,0,0.04)] border border-gray-100 flex flex-col justify-between h-28`}
  >
