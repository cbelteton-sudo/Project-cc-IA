...
export const FieldToday: React.FC = () => {
  ...
  useEffect(() => {
    const loadProjects = async () => {
      if (isOnline) {
        try {
          // Auth Header handled by api interceptor
    ...
    if (user || !isOnline) loadProjects();
  }, [isOnline, user]); // Add user dependency

  // Load Activities logic match existing...
  useEffect(() => {
    if (projects.length === 0) return;
    loadUnifiedActivities();
  }, [activeFilter, selectedDate, isOnline, projects, user]);
  ...
  const loadUnifiedActivities = async () => {
    ...
    const targets =
      activeFilter === 'ALL' ? projects : projects.filter((p) => p.id === activeFilter);

    for (const proj of targets) {
      let projectActs: Activity[] = [];
      if (isOnline) {
        try {
          const res = await api.get(`/activities/project/${proj.id}`);
  ...
  return (
    <div className="bg-gray-50 min-h-screen flex flex-col pb-20">
        ...
        <div className="flex justify-between items-start mb-6">
          <div>
            ...
            <h1 className="text-2xl font-bold text-gray-900 mt-1">
              {getGreeting()},{' '}
              <span className="text-blue-600">{user?.name?.split(' ')[0] || 'Usuario'}</span>
            </h1>
          </div>
      ...
      <div className="px-4 py-6 space-y-8">
        ...
        {!loading &&
          Object.entries(grouped).map(([pName, acts]) => (
                ...
                {acts.map((act) => {
                  ...
                  return (
                    <div
                      key={act.id}
                      onClick={() =>
                        navigate(`/field/entry/${act.id}`, { state: { projectId: act.projectId } })
                      }
                      ...
                      <div className="p-5">
                        ...
                        <div className="flex justify-between items-start mb-2">
                          <span className="text-[10px] font-bold text-gray-400 uppercase tracking-wider">
                            {act.parentName || 'General'}
                          </span>
                          <span
                            className={`text-[10px] font-bold px-2 py-0.5 rounded-full ${
                              statusClass.includes('red')
                        ...
                        <div className="flex items-center gap-4 text-xs text-gray-500 mb-3">
                          <div className="flex items-center gap-1">
                            <Calendar size={12} />
                            <span>
                              {new Date(act.startDate).toLocaleDateString('es-ES', {
                                day: 'numeric',
                                month: 'short',
                              })}{' '}
                              -{' '}
                              {new Date(act.endDate).toLocaleDateString('es-ES', {
                                day: 'numeric',
                                month: 'short',
                              })}
                            </span>
