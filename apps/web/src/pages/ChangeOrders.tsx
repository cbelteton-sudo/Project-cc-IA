...
export const ChangeOrders = () => {
  ...
  const { data: budgetData } = useQuery({
    ...
    queryFn: async () => (await api.get(`/budgets/${project?.budgets?.[0]?.id}/summary`)).data,
        ...
        {isLoading ? (
          ...
          <div className="divide-y divide-gray-50">
            {filteredCOs?.map((co) => (
              <div
                key={co.id}
                className="p-6 hover:bg-gray-50 transition border-l-4 border-transparent hover:border-blue-500"
              >
                ...
                <div className="bg-gray-50 rounded-lg p-3 space-y-2">
                  {co.items.map((item) => (
                    <div key={item.id} className="flex justify-between text-sm">
                      ...
                      <span
                        className={`font-mono font-medium ${item.amount >= 0 ? 'text-gray-700' : 'text-red-600'}`}
                      >
                    ...
                    >
                      <div>
                        <div className="font-medium text-sm text-gray-900">
                          {budgetData?.budgetLines?.find((l: any) => l.id === item.budgetLineId)
                            ?.name || 'Partida Desconocida'}
                        </div>
              ...
              <div className="flex justify-between items-center pt-4 border-t border-gray-100">
                <div className="text-lg">
                  ...
                  <span className="font-bold">
                    {currency(formData.items?.reduce((s, i) => s + i.amount, 0) || 0)}
                  </span>
                </div>
