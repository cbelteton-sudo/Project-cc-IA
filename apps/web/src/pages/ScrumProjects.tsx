...
import { CalendarClock, ArrowRight, Loader, Search, Folder, Calendar, Briefcase } from 'lucide-react';
...
export const ScrumProjects = () => {
    ...
    const { data: projects, isLoading } = useQuery({
        queryKey: ['projects'],
        queryFn: async () => (await axios.get(`${API_URL}/projects`, {
        ...
        return (
            <div className="container mx-auto max-w-7xl p-6">
                ...
                <div className="space-y-4">
                    {[1, 2, 3, 4, 5].map(i => <div key={i} className="h-16 bg-gray-100 rounded-xl animate-pulse"></div>)}
                </div>
            </div>
        );
    }
        ...
        <div className="container mx-auto max-w-7xl p-6 min-h-screen">
            ...
            <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 mb-8">
                <div>
                    ...
                    <p className="text-gray-500 mt-1">Gestiona Sprints, Backlogs y Tableros Kanban de tus proyectos.</p>
                </div>
                ...
                <div className="bg-white rounded-xl shadow-lg border-0 overflow-hidden ring-1 ring-black/5">
                    <table className="w-full text-left border-collapse">
                        <thead className="bg-field-blue text-white shadow-md z-10 relative">
                            <tr>
                                <th className="px-6 py-4 text-xs font-bold uppercase tracking-wider text-white/90">Proyecto</th>
                                <th className="px-6 py-4 text-xs font-bold uppercase tracking-wider text-white/90">Responsable</th>
                                <th className="px-6 py-4 text-xs font-bold uppercase tracking-wider text-center text-white/90">Estado</th>
                                <th className="px-6 py-4 text-xs font-bold uppercase tracking-wider text-white/90">Entrega</th>
                                <th className="px-6 py-4 text-xs font-bold uppercase tracking-wider text-white/90">Sprint Activo</th>
                                <th className="px-6 py-4 text-xs font-bold uppercase tracking-wider text-center text-white/90">Acciones</th>
                            </tr>
                        </thead>
                        <tbody className="divide-y divide-gray-100">
                            {filteredProjects?.map((project: Project) => {
                                ...
                                return (
                                    <tr key={project.id} className="hover:bg-blue-50/30 transition-colors group cursor-pointer" onClick={() => navigate(`/scrum/${project.id}`)}>
                                        ...
                                        <td className="px-6 py-4">
                                            <div className="text-sm text-gray-600">
                                                {project.managerName || <span className="text-gray-400 italic">No asignado</span>}
                                            </div>
                                        </td>
