...
const StatCard = ({ title, value, icon: Icon, color, subText }: any) => (
  <div className="bg-white p-6 rounded-xl shadow-sm border border-gray-100 flex items-center justify-between">
    ...
    <div
      className={`p-3 rounded-full ${color.replace('text-', 'bg-').replace('600', '50')} ${color}`}
    >
      <Icon size={24} />
    </div>
  </div>
...
export const PortalDashboard = () => {
  ...
  const { data: dashboard, isLoading } = useQuery({
    ...
    queryFn: async () => {
      const res = await api.get(`/portal/contractor/dashboard`);
      return res.data;
    },
    enabled: !!user,
  });
        ...
        <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-6">
          ...
          <div className="space-y-4">
              ...
              >
                <div>
                  ...
                  <p className="text-xs text-gray-500">
                    {task.project?.name} • Vence: {new Date(task.endDate).toLocaleDateString()}
                  </p>
                </div>
                <div className="text-right">
                  <span
                    className={`px-2 py-1 rounded text-xs font-bold ${
                      task.risk ? 'bg-red-100 text-red-700' : 'bg-blue-100 text-blue-700'
                    }`}
                  >
            ...
            {dashboard?.recentTasks?.length === 0 && (
              <p className="text-gray-400 text-sm">No hay tareas prioritarias.</p>
            )}
          </div>
        </div>
        ...
        <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-6">
          ...
          <div className="space-y-4">
            ...
            {dashboard?.recentOrders?.length === 0 && (
              <p className="text-gray-400 text-sm">No hay órdenes recientes.</p>
            )}
          </div>
        </div>
      </div>
