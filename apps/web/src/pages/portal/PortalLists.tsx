...
export const PortalProjects = () => {
  ...
  const { data: assignments } = useQuery({
    ...
    queryFn: async () => {
      const res = await api.get(`/portal/contractor/projects`);
      return res.data;
    },
    enabled: !!user,
  });
        ...
        {assignments?.map((a: any) => (
          <div key={a.id} className="bg-white p-6 rounded-xl shadow-sm border border-gray-100">
            ...
            <p className="text-sm text-gray-600 mb-4">
              Rol:{' '}
              <span className="font-medium text-blue-600">{a.roleInProject || 'Contratista'}</span>
            </p>
...
export const PortalTasks = () => {
  ...
  const { data: tasks } = useQuery({
    ...
    queryFn: async () => {
      const res = await api.get(`/portal/contractor/tasks`);
      return res.data;
    },
    enabled: !!user,
  });
                ...
                <td className="px-6 py-3">
                  ...
                  {t.risk && (
                    <span className="text-xs text-red-600 font-bold flex items-center gap-1">
                      <AlertCircle size={10} /> EN RIESGO
                    </span>
                  )}
                </td>
                <td className="px-6 py-3 text-sm text-gray-600">{t.project?.name}</td>
                <td className="px-6 py-3 text-xs text-gray-500">
                  del {new Date(t.startDate).toLocaleDateString()} al{' '}
...
export const PortalOrders = () => {
  ...
  const { data: orders } = useQuery({
    ...
    queryFn: async () => {
      const res = await api.get(`/portal/contractor/orders`);
      return res.data;
    },
    enabled: !!user,
  });
