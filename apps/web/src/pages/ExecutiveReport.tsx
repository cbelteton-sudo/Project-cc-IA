...
export const ExecutiveReport = () => {
  ...
  if (isLoading)
    return <div className="p-10 text-center animate-pulse">Generando Reporte Ejecutivo...</div>;
  if (error)
    return (
      <div className="p-10 text-center text-red-500">
        ...
        <p className="text-xs text-red-400">
          Posiblemente está deshabilitado para este proyecto o hubo un error de conexión.
        </p>
        <Link to={`/projects/${id}`} className="mt-4 inline-block text-blue-600 hover:underline">
          Volver al Proyecto
        </Link>
      </div>
    );

  const { project, kpis, narrative, evidence, issues, financials, sCurve } = report;
  ...
  return (
    <div className="min-h-screen bg-gray-50 print:bg-white">
      ...
      <div className="max-w-[210mm] mx-auto bg-white min-h-[297mm] p-8 md:p-12 shadow-lg print:shadow-none print:p-0 my-8 print:my-0">
        ...
        <header className="border-b border-gray-200 pb-6 mb-8 flex justify-between items-start">
          <div>
            ...
            <p className="text-slate-500 text-sm font-medium">
              {project.code} • Reporte de Progreso
            </p>
          </div>
          ...
          <div>
            ...
            <div className="space-y-4">
              ...
              <div className="flex justify-between items-center">
                ...
                <span
                  className={`font-bold ${kpis.issuesOpen > 0 ? 'text-red-600' : 'text-green-600'}`}
                >
                  {kpis.issuesOpen}
                </span>
              </div>
        ...
        {/* Financial & S-Curve Section (New) */}
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8 mb-10 break-inside-avoid-page">
          {/* Financial Summary */}
          <div className="bg-white p-6 rounded-xl border border-slate-100 shadow-sm print:border-slate-200">
            <div className="flex items-center gap-2 mb-4">
              <div className="p-2 bg-green-50 text-green-600 rounded-lg print:hidden">
                <DollarSign size={20} />
              </div>
            ...
            {financials && (
              <>
                <div className="grid grid-cols-2 gap-4 mb-6">
                  ...
                  <div>
                    <p className="text-xs text-slate-500 uppercase">Ejecutado</p>
                    <p className="text-lg font-bold text-blue-600">
                      {currency(financials.totalExecuted)}
                    </p>
                  </div>
                  <div>
                    <p className="text-xs text-slate-500 uppercase">Comprometido</p>
                    <p className="text-lg font-bold text-orange-600">
                      {currency(financials.totalCommitted)}
                    </p>
                  </div>
                  <div>
                    <p className="text-xs text-slate-500 uppercase">Por Gastar</p>
                    <p
                      className={`text-lg font-bold ${financials.remainingBudget >= 0 ? 'text-green-600' : 'text-red-500'}`}
                    >
                      {currency(financials.remainingBudget)}
                    </p>
                  </div>
                </div>
                  ...
                  <ResponsiveContainer width="100%" height="100%">
                    <BarChart
                      data={[
                        {
                          name: 'Total',
                          Budget: financials.totalBudget,
                          Executed: financials.totalExecuted,
                        },
                      ]}
                      layout="vertical"
                      barSize={20}
                    >
                      ...
                      <Tooltip formatter={(val: any) => currency(val)} />
                      <Legend />
                      <Bar
                        dataKey="Budget"
                        fill="#E2E8F0"
          ...
          <div className="bg-white p-6 rounded-xl border border-slate-100 shadow-sm print:border-slate-200">
            ...
            {sCurve && sCurve.length > 0 ? (
              <div className="h-64 w-full">
                <ResponsiveContainer width="100%" height="100%">
                  ...
                  >
                    ...
                    <Tooltip
                      formatter={(val: any) => currency(val)}
              ...
              <div className="h-64 w-full flex items-center justify-center text-gray-400 italic text-sm">
                Se requieren datos históricos para la Curva S.
              </div>
            )}
          </div>
        </div>
        ...
        <div className="grid grid-cols-4 gap-4 mb-10">
          <div className="p-4 rounded-lg bg-orange-50 border border-orange-100 print:border-gray-200 print:bg-white text-center">
            ...
            <p className="text-xs font-semibold text-orange-800 uppercase print:text-gray-500">
              Estancadas ({'>'}3d)
            </p>
          </div>
          <div className="p-4 rounded-lg bg-red-50 border border-red-100 print:border-gray-200 print:bg-white text-center">
            ...
            <p className="text-xs font-semibold text-red-800 uppercase print:text-gray-500">
              Bloqueadas
            </p>
          </div>
          <div className="p-4 rounded-lg bg-yellow-50 border border-yellow-100 print:border-gray-200 print:bg-white text-center">
            ...
            <p className="text-xs font-semibold text-yellow-800 uppercase print:text-gray-500">
              Punch List
            </p>
          </div>
          <div className="p-4 rounded-lg bg-slate-50 border border-slate-100 print:border-gray-200 print:bg-white text-center">
            <p className="text-3xl font-bold text-slate-600 print:text-black">
              {kpis.issuesOverdue}
            </p>
            <p className="text-xs font-semibold text-slate-800 uppercase print:text-gray-500">
              Vencidos
            </p>
          </div>
        </div>
        ...
        <div className="print:block hidden mb-6 print:mb-2">
          <h2 className="text-xl font-bold border-b-2 border-slate-800 pb-2 mb-4">
        ...
        <div className="print:hidden border-b border-gray-200 mb-6 flex gap-6">
          <button
            ...
            className={`pb-3 text-sm font-medium transition ${activeTab === 'evidence' ? 'border-b-2 border-blue-600 text-blue-600' : 'text-gray-500 hover:text-gray-800'}`}
          >
            Evidencia ({evidence.length})
          </button>
          <button
            onClick={() => setActiveTab('issues')}
            className={`pb-3 text-sm font-medium transition ${activeTab === 'issues' ? 'border-b-2 border-blue-600 text-blue-600' : 'text-gray-500 hover:text-gray-800'}`}
          >
            Punch List ({issues.length})
          </button>
        </div>

        {/* Evidence Section */}
        <div
          className={`${activeTab === 'evidence' ? 'block' : 'hidden'} print:block mb-10 break-inside-avoid-page`}
        >
          <div className="space-y-6">
            {evidence.map((entry: any) => (
              <div
                key={entry.id}
                className="flex gap-4 border-b border-gray-100 pb-6 break-inside-avoid"
              >
                {/* Date Column */}
            ...
            {evidence.length === 0 && (
              <p className="text-center text-gray-400 italic">No evidence found in this period.</p>
            )}
          </div>
        </div>
              ...
              {issues.map((issue: any) => (
                <tr key={issue.id} className="break-inside-avoid">
                  ...
                  <td className="py-2 px-3 text-gray-600 font-mono text-xs">
                    {issue.dueDate ? format(new Date(issue.dueDate), 'dd/MM') : '-'}
                  </td>
