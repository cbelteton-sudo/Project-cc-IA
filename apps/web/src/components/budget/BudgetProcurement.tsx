...
export const BudgetProcurement = ({ projectId, currency = 'USD' }: BudgetProcurementProps) => {
  const { data: pos, isLoading: isLoadingPOs } = useQuery({
    ...
    queryFn: async () => {
      const { data } = await api.get(`/purchase-orders?projectId=${projectId}`);
      return data;
    },
  });
  ...
  const { data: invoices, isLoading: isLoadingInvoices } = useQuery({
    ...
    queryFn: async () => {
      const { data } = await api.get(`/invoices?projectId=${projectId}`);
      return data;
    },
  });
  ...
  if (isLoadingPOs || isLoadingInvoices)
    return <div className="p-8 text-center text-gray-500">Cargando adquisiciones...</div>;
    ...
    <div className="space-y-8">
      ...
      <div className="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden">
        <div className="px-6 py-4 border-b border-gray-100 flex justify-between items-center bg-green-50">
          ...
          <Link to="/invoices" className="text-sm text-green-600 hover:text-green-700 font-medium">
            Ver Todas
          </Link>
        </div>
        <div className="overflow-x-auto">
            ...
            <tbody className="divide-y divide-gray-100">
              {invoices?.map((inv: any) => (
                <tr key={inv.id} className="hover:bg-gray-50">
                  ...
                  <td className="px-6 py-3">
                    <span
                      className={`px-2 py-0.5 rounded-full text-xs font-medium ${getStatusColor(inv.status)}`}
                    >
                      {inv.status}
                    </span>
                  </td>
