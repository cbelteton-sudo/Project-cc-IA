...
const SidebarItem = ({ to, icon: Icon, label, collapsed }: any) => (
  <Link
    to={to}
    className={`flex items-center gap-3 px-3 py-2 text-gray-300 hover:bg-white/10 hover:text-white rounded-md transition-colors ${collapsed ? 'justify-center' : ''}`}
...
export const Layout = () => {
  ...
  return (
    <div className="flex h-screen w-full bg-gray-50 print:h-auto print:block">
      {/* Sidebar */}
      <aside
        className={`${collapsed ? 'w-20' : 'w-64'} bg-field-blue border-r border-white/5 flex flex-col print:hidden transition-all duration-300 relative`}
      >
        {/* Toggle Button */}
        <button
        ...
        <div
          className={`h-16 flex items-center ${collapsed ? 'justify-center px-0' : 'px-6'} border-b border-white/5 bg-field-blue transition-all`}
        >
          <div className="flex items-center gap-2">
            {/* Logo Icon - Checkmark REMOVED */}
            {!collapsed && (
              <span className="font-bold text-white text-lg tracking-tight">
                FIELD<span className="text-field-orange">CLOSE</span>
              </span>
            )}
            {collapsed && <span className="font-bold text-white text-lg tracking-tight">FC</span>}
          </div>
        </div>
        ...
        <nav className="flex-1 p-2 space-y-1 overflow-y-auto overflow-x-hidden">
          ...
          {['ADMIN', 'ADMINISTRADOR', 'DIRECTOR'].includes(user?.role || '') && (
            <>
              {!collapsed && (
                <div className="pt-4 pb-1 pl-3 text-xs font-semibold text-slate-500 uppercase tracking-wider transition-opacity duration-200">
                  {t('sidebar.projectMgmt')}
                </div>
              )}
              <SidebarItem
                to="/projects"
              ...
              <SidebarItem
                ...
                icon={ShoppingCart}
                label={t('sidebar.materialRequests')}
                collapsed={collapsed}
              />
              <SidebarItem
              ...
              <SidebarItem
                to="/reports"
                icon={FileText}
                label="Centro Reportes"
                collapsed={collapsed}
              />
            </>
          )}

          {['ADMIN', 'ADMINISTRADOR', 'SUPERVISOR', 'RESIDENTE', 'USER', 'PM', 'DIRECTOR'].includes(
            user?.role || '',
          ) && (
            <>
              {!collapsed && (
              ...
              {['ADMIN', 'ADMINISTRADOR', 'PM', 'DIRECTOR', 'SUPERVISOR'].includes(
                user?.role || '',
              ) && (
                <SidebarItem
                  to="/field/dashboard"
