...
export const DailyUpdateLogModal = ({
  isOpen,
  onClose,
  projectId,
  backlogItemId,
  backlogItemTitle,
}: DailyUpdateLogModalProps) => {
  ...
  const { data: updates, isLoading } = useQuery({
    ...
    queryFn: async () => {
      const res = await api.get(`/scrum/daily-updates/${projectId}?backlogItemId=${backlogItemId}`);
      return res.data;
    },
    enabled: isOpen,
  });
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      <div>
                        ...
                        <p className="text-xs text-gray-500 flex items-center gap-1">
                          <Calendar size={10} />
                          {format(new Date(update.createdAt), "d 'de' MMMM, yyyy - h:mm a", {
                            locale: es,
                          })}
                        </p>
                      </div>
