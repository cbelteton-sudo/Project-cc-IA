...
export const SprintPlanning = ({ projectId, onSprintStarted }: { projectId: string, onSprintStarted: () => void }) => {
    ...
    const createSprintMutation = useMutation({
        mutationFn: async (data: any) => axios.post(`${API_URL}/scrum/sprints`, {
            ...data,
            projectId,
            createdByUserId: user?.id || user?.userId || '4044f19e-764f-4109-9f73-d590b61116ca'
        }),
        onSuccess: () => {
    ...
    const addToSprintMutation = useMutation({
        mutationFn: async () => {
            ...
            return axios.post(`${API_URL}/scrum/sprints/${targetSprint.id}/items`, { items: selectedItems });
        },
        onSuccess: () => {
            queryClient.invalidateQueries({ queryKey: ['scrum', 'sprints', projectId] });
    ...
    const startSprintMutation = useMutation({
        mutationFn: async (sprintId: string) => axios.patch(`${API_URL}/scrum/sprints/${sprintId}/start`),
        onSuccess: () => {
            queryClient.invalidateQueries({ queryKey: ['scrum', 'sprints', projectId] });
            onSprintStarted();
        }
    ...
    const availableBacklog = backlogItems?.filter((i: any) => {
        const status = i.status || 'BACKLOG';
        return !['IN_SPRINT', 'PENDING_PLANNING', 'DONE', 'COMPLETED', 'ARCHIVED'].includes(status) && !i.isVirtual;
    }) || [];
        ...
        <div className="h-full flex gap-4">
            ...
            <div className="flex-1 flex flex-col bg-white rounded-xl border border-gray-200 overflow-hidden">
                <div className="p-4 border-b border-gray-100 bg-gray-50 flex justify-between items-center">
                    ...
                    <span className="bg-gray-200 text-gray-600 px-2 py-0.5 rounded-full text-xs font-bold">{availableBacklog.length}</span>
                </div>
            ...
            {/* Sprint Column */}
            <div className={`flex-1 flex flex-col rounded-xl border overflow-hidden ${isTargetActive ? 'bg-green-50 border-green-200' : 'bg-white border-gray-200'}`}>
                <div className={`p-4 border-b flex justify-between items-center ${isTargetActive ? 'bg-green-100 border-green-200' : 'bg-indigo-50 border-gray-100'}`}>
                    <div>
                        ...
                        {targetSprint && <p className={`text-xs ${isTargetActive ? 'text-green-700' : 'text-indigo-700'}`}>{targetSprint.name}</p>}
                    </div>
                ...
                <div className={`flex-1 overflow-y-auto p-2 space-y-2 ${isTargetActive ? 'bg-green-50/30' : 'bg-indigo-50/30'}`}>
                    {targetSprint?.items?.length > 0 ? (
                        targetSprint.items.map((sItem: any) => (
                            <div key={sItem.id} className="p-3 bg-white border border-gray-100 rounded-lg shadow-sm">
                        ...
                        <div className="h-full flex items-center justify-center text-gray-400 text-sm italic p-8 text-center">
                            {!targetSprint ? "Crea un sprint para comenzar" : (isTargetActive ? "Este sprint no tiene ítems asignados aún." : "Arrastra o selecciona ítems del backlog")}
                        </div>
                    )}
                </div>
            </div>
...
const CreateSprintModal = ({ onClose, onCreate }: { onClose: () => void, onCreate: (data: any) => void }) => {
        ...
        <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4">
            <div className="bg-white rounded-xl shadow-2xl w-full max-w-md p-6">
                ...
                <form onSubmit={handleSubmit} className="space-y-4">
                    <div>
                        <label className="block text-sm font-medium text-gray-700 mb-1">Nombre del Sprint</label>
                        <input
                            type="text"
                            required
                            value={name}
                    ...
                    <div className="flex justify-end gap-3 pt-2">
                        <button type="button" onClick={onClose} className="px-4 py-2 text-gray-700 hover:bg-gray-100 rounded-lg font-medium transition-colors">Cancelar</button>
                        <button type="submit" className="px-4 py-2 bg-blue-600 text-white rounded-lg font-medium hover:bg-blue-700 transition-colors shadow-sm">Crear Sprint</button>
                    </div>
                </form>
            </div>
        </div>
