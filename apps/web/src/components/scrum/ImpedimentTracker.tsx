...
export const ImpedimentTracker = ({ projectId }: { projectId: string }) => {
  ...
  const { data: impediments, isLoading } = useQuery({
    ...
    queryFn: async () => (await api.get(`/scrum/impediments/${projectId}`)).data,
  });

  const resolveMutation = useMutation({
    mutationFn: async (id: string) => api.patch(`/scrum/impediments/${id}/resolve`),
    onSuccess: () =>
      queryClient.invalidateQueries({ queryKey: ['scrum', 'impediments', projectId] }),
  });

  const createMutation = useMutation({
    mutationFn: async (data: any) => api.post(`/scrum/impediments`, { ...data, projectId }),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['scrum', 'impediments', projectId] });
  ...
  if (isLoading)
    return <div className="p-8 text-center text-gray-400">Cargando impedimentos...</div>;
        ...
        <div className="flex-1 overflow-y-auto p-4 space-y-3">
          {activeImpediments.length === 0 ? (
            <p className="text-center text-gray-400 italic py-10">
              No hay impedimentos activos. ¡Buen trabajo!
            </p>
          ) : (
                ...
                <div className="flex justify-between items-start">
                  <div>
                    ...
                    <div className="flex gap-4 mt-3 text-xs text-gray-500">
                      ...
                      <span
                        className={`font-bold ${imp.severity > 2 ? 'text-red-600' : 'text-orange-500'}`}
                      >
          ...
          {resolvedImpediments.map((imp: Impediment) => (
            <div key={imp.id} className="p-3 border border-gray-200 bg-white rounded-lg opacity-70">
              ...
              <p className="text-xs text-green-600 mt-1">
                Resuelto el {format(new Date(imp.resolvedAt!), 'd MMM', { locale: es })}
              </p>
            </div>
          ))}
        </div>
              ...
              <div className="space-y-4">
                <div>
                  ...
                  <input
                    ...
                    placeholder="Ej: Falta de material en zona norte"
                  />
                </div>
                <div>
