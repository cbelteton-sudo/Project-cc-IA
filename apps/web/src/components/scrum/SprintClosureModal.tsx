...
export const SprintClosureModal = ({
  ...
  const closeMutation = useMutation({
    mutationFn: async () => {
      await api.patch(`/scrum/sprints/${sprintId}/close`, {
        keep,
        improve,
        stop,
        teamMood: teamMood[0],
        ...
        <div className="p-6 border-b border-gray-100 bg-indigo-50 flex justify-between items-start">
          <div>
            ...
            <p className="text-sm text-indigo-600 mt-1">
              Cierra el ciclo y documenta aprendizajes.
            </p>
          </div>
          <button
            onClick={onClose}
            className="text-gray-400 hover:text-gray-600 rounded-full p-1 hover:bg-indigo-100 transition-colors"
          >
            <X size={24} />
          </button>
        </div>
        ...
        <div className="p-8 overflow-y-auto">
          ...
          <div className="grid grid-cols-3 gap-6 mb-8">
            ...
            <div className="bg-blue-50 border border-blue-100 rounded-xl p-4 text-center relative overflow-hidden">
              ...
              ></div>
              <div className="text-3xl font-bold text-blue-600 mb-1 relative">
                {completionPercentage}%
              </div>
              <div className="text-xs font-bold text-blue-700 uppercase tracking-wide relative">
                Eficiencia
              </div>
            </div>
          </div>
