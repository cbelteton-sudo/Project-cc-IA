...
export const GanttWBS = () => {
    const { visibleActivities, headerHeight, rowHeight, expanded, toggleExpand, onSelectActivity } = useGantt();
    ...
    return (
        <div className="flex flex-col h-full overflow-hidden">
            ...
            >
                <div className="flex-1 px-4 border-r border-gray-200 h-full flex items-center">Actividad</div>
                ...
                {visibleActivities.map((act) => (
                    ...
                    >
                        ...
                        >
                            ...
                            {act.hasChildren ? (
                                ...
                                <div className="w-6 mr-1 flex items-center justify-center">
                                    <div className={`w-2 h-2 transform rotate-45 
                                        ${act.status === 'ACHIEVED' ? 'bg-green-500' :
                                            (act.status === 'MISSED' || new Date(act.startDate) < new Date()) ? 'bg-red-500' :
                                                'bg-gray-400'}
                                    `}></div>
                                </div>
                            ) : (
                            ...
                            <span className={`truncate ml-2 font-medium group-hover:text-blue-700 ${act.type === 'MILESTONE' ? 'text-purple-700 font-bold' : 'text-gray-700'}`}>
                                {act.name}
                            </span>
                        </div>
