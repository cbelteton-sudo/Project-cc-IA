...
import { CheckCircle, Calendar, User, TrendingUp, Clock, AlertTriangle, Flag, X, GitMerge, History, ChevronDown, ArrowRight, Trash2, Link as LinkIcon, Camera } from 'lucide-react';
...
export const ActivityDetails = ({ activityId, token, onUpdate, onCreateSubActivity, onClose }: ActivityDetailsProps) => {
    ...
    const { data: users } = useQuery({
        queryKey: ['users'],
        queryFn: async () => (await axios.get(`${API_URL}/users`, { headers: { Authorization: `Bearer ${token}` } })).data
    });
                ...
                <div className="max-w-2xl mx-auto space-y-3">
                    ...
                    <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-4">
                        <div className="flex justify-between items-end mb-3">
                            <div>
                                <h3 className="text-[10px] font-bold text-gray-500 uppercase tracking-wider mb-0.5">Progreso Actual</h3>
                                <div className="text-3xl font-extrabold text-blue-600">{activity.percent}%</div>
                            </div>
                            <div className="text-right">
                                <div className="mb-2">
                                    <span className={`px-2 py-0.5 text-[10px] font-bold uppercase tracking-wider rounded-md ${activity.status === 'DONE' ? 'bg-emerald-100 text-emerald-800' :
                                ...
                                <h3 className="text-[10px] font-bold text-gray-400 uppercase tracking-wider mb-0.5">Fechas</h3>
                    ...
                    <div className="bg-blue-50/50 rounded-lg shadow-sm border border-blue-100 p-4">
                        ...
                        <div className="space-y-3">
                            <div>
                                <label className="block text-[10px] font-bold text-blue-800 mb-1">Nota / Comentario <span className="text-red-500">*</span></label>
                                <textarea
                                    rows={3}
                                    value={notes}
                    ...
                    <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-4">
                        <h3 className="text-[10px] font-bold text-gray-500 uppercase tracking-wider mb-3 border-b border-gray-100 pb-1.5">Equipo responsable de ejecución</h3>

                        <div className="grid grid-cols-2 gap-4">
                            {/* Contractor */}
                            <div>
                                <span className="block text-[10px] text-gray-400 font-bold uppercase mb-1">Contratista</span>
                                <div className="flex items-center gap-2 p-2 bg-gray-50 rounded-md border border-gray-100">
                                    <div className="w-8 h-8 rounded-full bg-white border border-gray-200 flex items-center justify-center text-base shadow-sm">🏢</div>
                                    <div className="min-w-0">
                                        <div className="font-bold text-gray-900 text-xs truncate">{activity.contractor?.name || 'No asignado'}</div>
                            ...
                            <div>
                                <span className="block text-[10px] text-gray-400 font-bold uppercase mb-1">Supervisor</span>
                                <div className="flex items-center gap-2 p-2 bg-gray-50 rounded-md border border-gray-100">
                                    <div className="w-8 h-8 rounded-full bg-white border border-gray-200 flex items-center justify-center text-base shadow-sm">👷</div>
                                    <div className="min-w-0">
                                        <div className="font-bold text-gray-900 text-xs truncate">
                                            {(activity as any).assignedUser?.name || (activity as any).backlogItems?.[0]?.assigneeUser?.name || 'No asignado'}
                                        </div>
                                        <div className="text-[10px] text-gray-400 italic">Asignar en Sprint Backlog</div>
                                    </div>
                                </div>
                            </div>
                        </div>
                    ...
                    <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-4">
                        <h3 className="text-[10px] font-bold text-gray-500 uppercase tracking-wider mb-3 border-b border-gray-100 pb-1.5">Lógica y Presupuesto</h3>

                        <div className="space-y-4">
                            {/* Dependencies */}
                            <div>
                                <label className="block text-[10px] font-bold text-gray-700 mb-1.5">Predecesoras</label>
                                <div className="space-y-1.5 mb-2">
                                    {activity.dependencies?.map((dep: any) => (
                                        <div key={dep.id} className="flex justify-between items-center text-xs bg-orange-50 text-orange-900 px-3 py-1.5 rounded border border-orange-100">
                                            <span className="font-medium flex items-center gap-1.5"><AlertTriangle size={12} /> {dep.dependsOn?.name}</span>
                                            <button onClick={() => handleRemoveDependency(dep.id)} className="text-orange-400 hover:text-orange-700"><X size={14} /></button>
                                        </div>
                                    ))}
                                    {(!activity.dependencies?.length) && <p className="text-xs text-gray-400 italic">Sin dependencias.</p>}
                                </div>
                                ...
                                <div className="flex gap-2">
                                    ...
                                    >
                                        ...
                                        {allActivities?.map((act: any) => (
                                            act.id !== activity.id && (<option key={act.id} value={act.id}>{act.name}</option>)
                                        ))}
                                    </select>
                                    <button
                            ...
                            {/* Budget */}
                            <div>
                                <label className="block text-[10px] font-bold text-gray-700 mb-1.5">Renglón Presupuestario</label>
                                <select
                    ...
                    <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-4">
                        <div className="flex justify-between items-center mb-4 border-b border-gray-100 pb-1.5">
                            <h3 className="text-[10px] font-bold text-gray-500 uppercase tracking-wider">Historial</h3>
                        ...
                        <div className="relative pl-3 space-y-4">
                            ...
                            {displayedHistory.map((record: any, idx: number) => {
                                ...
                                const dateStr = safeDateTime(record.date || record.createdAt || record.weekStartDate);
                                ...
                                if (isDailyUpdate) {
                                    ...
                                    contentText = record.notes || (record.qtyDone ? `Cant: ${record.qtyDone}` : 'Estado actualizado');
                                ...
                                return (
                                    <div key={idx} className="relative flex gap-3">
                                        <div className={`relative z-10 w-3 h-3 mt-1 rounded-full border-2 border-white shadow-sm shrink-0 ${badgeColor}`}></div>
                            ...
                            {(!activity.progressRecords?.length && !activity.dailyUpdates?.length && !activity.fieldUpdates?.length) && (
