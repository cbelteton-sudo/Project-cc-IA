...
async function main() {
  ...
  const activeProject = projects.sort((a, b) => {
    const dateA =
      a.costLedger[0]?.date || a.timesheets[0]?.createdAt || new Date(0);
    const dateB =
      b.costLedger[0]?.date || b.timesheets[0]?.createdAt || new Date(0);
  ...
  if (laborEntries.length === 0) {
    console.log(
      '   ❌ No Labor entries found in the Ledger. The Timesheet Service is NOT writing to the DB.',
    );
    ...
    const totalHours = timesheets.reduce(
      (sum, t) => sum + t.entries.reduce((s, e) => s + Number(e.hours), 0),
      0,
    );
    console.log(
      `   ⚠️ However, found ${timesheets.length} timesheets with ${totalHours} total hours.`,
    );
    return;
  }
  ...
  for (const entry of laborEntries) {
    const exactMatch = budget.budgetLines.find(
      (l) =>
        l.wbsActivityId === entry.wbsActivityId &&
        l.costType === entry.costType,
    );

    const activityOnlyMatch = budget.budgetLines.find(
      (l) => l.wbsActivityId === entry.wbsActivityId,
    );

    if (exactMatch) {
      console.log(
        `  ✅ [${entry.date.toISOString().split('T')[0]}] Entry ${entry.id.substring(0, 8)} => Matches Line "${exactMatch.code}" (CostType: ${entry.costType})`,
      );
      matchCount++;
    } else {
      console.log(
        `  ❌ [${entry.date.toISOString().split('T')[0]}] Entry ${entry.id.substring(0, 8)} => NO JOIN POSSIBLE`,
      );
      console.log(
        `     Entry Values: Activity=${entry.wbsActivityId}, CostType='${entry.costType}'`,
      );
      if (activityOnlyMatch) {
        console.log(
          `     💡 Reason: Mismatched CostType. Budget expects '${activityOnlyMatch.costType}' but Ledger has '${entry.costType}'`,
        );
      } else {
        console.log(
          `     💡 Reason: Activity ID ${entry.wbsActivityId} not found in Budget Lines.`,
        );
      }
      mismatchCount++;
    }
