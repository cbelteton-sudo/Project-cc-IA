...
async function main() {
  ...
  if (!activeSprint) {
    ...
    if (activeSprint.items.length === 0) {
      ...
      activeSprint.items.forEach((item) => {
        console.log(
          `- Item: ${item.backlogItem.title} | Status: ${item.boardStatus} | BacklogStatus: ${item.backlogItem.status}`,
        );
      });
    }
  }
  ...
  if (plannedSprint) {
    console.log(
      `Planned Sprint: ${plannedSprint.name} | Items: ${plannedSprint._count.items}`,
    );
  }
}
