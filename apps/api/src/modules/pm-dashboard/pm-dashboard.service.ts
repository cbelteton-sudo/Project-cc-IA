...
export class PMDashboardService {
  ...
  async getDashboardData(projectId: string, tenantId: string) {
    ...
    if (!project.enablePMDashboard)
      throw new ForbiddenException('PM Dashboard disabled for this project');
    ...
    const atRiskIssues = issues.filter(
    ...
    const contractorStats = new Map<
      string,
      { name: string; blocked: number; overdue: number }
    >();

    blockedActivities.forEach((a) => {
      if (a.contractor) {
        const current = contractorStats.get(a.contractor.id) || {
    ...
    overdueIssues.forEach((i) => {
      if (i.contractor) {
        const current = contractorStats.get(i.contractor.id) || {
          name: i.contractor.name,
          blocked: 0,
          overdue: 0,
        };
        current.overdue++;
