...
export class DailyLogService {
  ...
  async getDailyLog(projectId: string, dateStr: string, tenantId: string) {
  ...
  async generatePdf(
    ...
    const { updates, issues } = await this.getDailyLog(
      projectId,
      dateStr,
      tenantId,
    );
    ...
    return new Promise((resolve) => {
        ...
        updates.forEach((update) => {
          update.items.forEach((item) => {
            ...
            doc
              .font('Helvetica')
              .text(
                `Progress: ${item.manualPercent ?? item.suggestedPercent}% (Confidence: ${item.confidenceScore ?? 'N/A'})`,
              );
            if (item.notes) doc.text(`Notes: ${item.notes}`);
            if (item.isRisk)
              doc.fillColor('red').text('Status: AT RISK').fillColor('black');
            doc.moveDown(0.5);
          });
        });
      }
      ...
      if (issues.length === 0) {
        ...
        issues.forEach((issue) => {
          ...
          if (issue.activity)
            doc.text(`Related Activity: ${issue.activity.name}`);
