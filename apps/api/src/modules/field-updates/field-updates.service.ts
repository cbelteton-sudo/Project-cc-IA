...
export class FieldUpdatesService {
  ...
  private async processItem(fieldUpdateId: string, dto: UpdateItemDto) {
    ...
    const suggestedPercent = await this.calculateSuggestedPercent(
      activity,
      dto,
    );
    ...
    if (
      dto.manualPercent !== undefined &&
      Math.abs(dto.manualPercent - suggestedPercent) > 20
    ) {
      requiresReview = true;
    }
    ...
    const data = {
      ...
      confidenceScore,
      requiresReview,
      status: dto.status, // GAP FIX
    };

    // GAP FIX: Update Parent Activity Real-time
    if (dto.status || dto.manualPercent !== undefined) {
      const updateData: any = {};
      if (dto.status) updateData.status = dto.status;
      if (dto.manualPercent !== undefined)
        updateData.percent = dto.manualPercent;

      // Auto-complete logic
      if (dto.status === 'DONE') {
        updateData.percent = 100;
        updateData.manualProgress = 100; // For Gantt
      }
  ...
  private async calculateSuggestedPercent(
    activity: any,
    dto: UpdateItemDto,
  ): Promise<number> {
    ...
    if (type === 'CHECKLIST' || type === 'MILESTONE') {
      ...
      const state = dto.checklistState
        ? JSON.parse(dto.checklistState)
        : dto.milestoneState
          ? JSON.parse(dto.milestoneState)
          : {};
  ...
  private async calculateConfidenceScore(
    ...
    const lastPercent =
      lastUpdate?.manualPercent || lastUpdate?.suggestedPercent || 0;
    const currentPercent =
      dto.manualPercent !== undefined ? dto.manualPercent : suggestedPercent;
