...
export class ContractorPortalController {
  ...
  async getProfile(@ActiveUser() user: any) {
    if (!user.contractorId)
      throw new UnauthorizedException('Not a contractor user');
  ...
  async getProjects(@ActiveUser() user: any) {
    if (!user.contractorId)
      throw new UnauthorizedException('Not a contractor user');
    return this.contractorsService.getAssignments(
      user.tenantId,
      user.contractorId,
    );
  }
  ...
  async getDashboard(@ActiveUser() user: any) {
    if (!user.contractorId)
      throw new UnauthorizedException('Not a contractor user');
    return this.contractorsService.getPortalDashboard(
      user.tenantId,
      user.contractorId,
    );
  }
  ...
  async getTasks(@ActiveUser() user: any, @Query('status') status?: string) {
    if (!user.contractorId)
      throw new UnauthorizedException('Not a contractor user');
    return this.contractorsService.getPortalTasks(
      user.tenantId,
      user.contractorId,
      status,
    );
  ...
  async getPurchaseOrders(@ActiveUser() user: any) {
    if (!user.contractorId)
      throw new UnauthorizedException('Not a contractor user');
    return this.contractorsService.getPortalOrders(
      user.tenantId,
      user.contractorId,
    );
  }
