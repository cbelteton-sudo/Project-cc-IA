...
async function main() {
  ...
  const createActivity = async (
    ...
    return prisma.projectActivity.create({
      data: {
        ...
        status: 'NOT_STARTED',
        contractorId:
          contractorIdx !== undefined
            ? contractors[contractorIdx].id
            : undefined,
      },
    });
  };
  ...
  const actA3 = await createActivity(
  ...
  const actE1 = await createActivity(
